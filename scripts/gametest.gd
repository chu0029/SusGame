...
func CreatePower() -> void: #power created from clicking the thing
	power += 1 * multiplier
	totalUpgCount += 1*multiplier
	susUpgCount += 1*multiplier

func AutoFactory() -> void: 
	sustainability = (susUpgCount*1.2/totalUpgCount)*100 #1.2 for balencing issues
	#print(susUpgCount, '|', totalUpgCount, '|', sustainability)
	if sustainability < 70: #and sustainability > 0 ?
		health -= 1
	susUpgCount += ( #amt of power generated by sus upgrades
		solarpanelcount + windturbinecount*2 + biomasscount*5 + geothermalplantcount*15 + 
		hydroelectricdamcount*50 + nuclearfusioncount*1000
	)
	totalUpgCount += ( #amt of total power
		solarpanelcount + windturbinecount*2 + biomasscount*5 + geothermalplantcount*15 + 
		hydroelectricdamcount*50 + nuclearfusioncount*1000 + nonsuscount[0] + 
		nonsuscount[1]*2 + nonsuscount[2]*5 + nonsuscount[3]*15 + nonsuscount[4]*50 + 
		nonsuscount[5]*1000
...
func _on_windturbine_pressed() -> void:
	if power >= costs[1]:
		var output = upgradeClicker(costs[1], windturbinecount)
		costs[1] = output[0]
